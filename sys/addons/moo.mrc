...
alias moo {
  ...
  if ($1 == echo) { 
    if ($moof($2)) { var %moo.var $ifmatch | echo -a %moo.var }
    else { echo -a $moor }
    return
  }
  if ($moof($1)) { var %moo.var $ifmatch | $iif($active == Status Window,echo -gta,$iif($chan,msg $chan,say)) %moo.var }
}
...
alias -l moor return $iif($mooi(name),$moof(os) $moof(up) $moof(cpu) $moof(gfx) $moof(res) $moof(ram) $moof(hdd) $moof(net),lookup error)
...
alias mooi {
  ...
  if ($1 == hdd) { 
    var %i 1, %dtotal 0, %dfree 0 
    while (%i <= $disk(0)) { 
      if ($disk(%i).type == fixed) {
        var %var %var $+(,$remove($disk(%i).path,\),) $+($bytes($disk(%i).free).suf,/,$bytes($disk(%i).size).suf) --
        var %d1 $calc(%d1 + $disk(%i).free)
        var %d2 $calc(%d2 + $disk(%i).size)
      }
      inc %i 
    } 
    var %var %var Total: $+($bytes(%d1).suf,/,$bytes(%d2).suf)
    return %var 
  }
