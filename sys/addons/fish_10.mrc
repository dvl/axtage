...
on ^*:NOTICE:DH1080_INIT*:?:{
  if ($len($2) > 178 && $len($2) < 182) {
    query $nick
    echo $color(Mode text) -tm $nick $m FiSH03:01 Received DH1080 public key from $nick $+ , sending mine...
...
alias -l FiSH.WriteKey {
  ...
  var %info = $m FiSH03:01 Key for %cur_contact set to *censored* $iif($gettok($3-,1,58) == cbc, (CBC Mode), )
...
alias FiSH.usechankey {
  ...
  if (%theKey == $null) {
    echo $color(Mode text) -at $m FiSH03:01 No valid key for $2 found
  }
  else {
    writeini -n %blow_ini $1 key %theKey
    writeini -n %blow_ini $1 date $date
    unset %theKey
    echo $color(Mode text) -at $m FiSH03:01 Using same key as $2 for $1
  }
}
...
alias FiSH.showkey {
  ...
  if (%theKey != $null) {
    ...
    echo $color(Mode text) -at $m FiSH03:01 No valid key for %cur_contact found
  }
}
...
alias FiSH.removekey {
  ...
  echo $color(Mode text) -at $m FiSH03:01 Key for %cur_contact has been removed
}
...
alias FiSH.DH1080_INIT {
  ...
  echo $color(Mode text) -tm $nick $m FiSH03:01 Sent my DH1080 public key to %cur_contact $+ , waiting for reply ...
}
...
alias FiSH.prefix {
  if ($1 != $null) {
    ...
    flushini mirc.ini
    echo $color(Mode text) -at $m FiSH03:01 Plain-prefix set to $1-
  }
}
...
menu status,channel,nicklist,query {
  ...
  ..-
  ..FiSH 10:run http://github.com/flakes/mirc_fish_10
}
